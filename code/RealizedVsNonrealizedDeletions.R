...
library(here)
library(ggpubr)
plots_dir <- here("3_Results") %>% normalizePath()
### 1: READ VICTOR's FILE
Rep <- read.table(
    here("2_Derived/Homo_sapiens_triangles.txt"),
    sep = "\t",
    header = FALSE
)
names(Rep) <- c(
...
# mb_del == realised deletions (exist in MitoBreak); non_del - non realised deletions == don't exist in MitoBreak
...
##
pdf(here(plots_dir, "RealizedVsNonrealizedDeletions.R.01.pdf"))
