...
library(tidyverse)
library(here)
plots_dir <- here("3_Results") %>% normalizePath() # Windows-people care

here("3_Results/SlipAndJump.R.01.pdf") %>%
    normalizePath() %>%
    pdf()

##### 1: READ microhomology from pair-wise alignments
homol <- read.table(
    here("2_Derived/HeatMaps/100x100.csv") %>%
        normalizePath(),
    sep = ";",
    header = TRUE
)
...
DirectRepDensity <-
    here("2_Derived/HeatMaps/Link_matrix_direct_major_activ_left.csv") %>%
    normalizePath() %>%
    read.table(
        sep = ";",
        header = TRUE
...
breaks <- read.table(
    here("1_Raw/MitoBreakDB_12122019.csv") %>%
        normalizePath(),
    sep = ",",
    header = TRUE
)
...
GlobalFolding <- here("2_Derived/HeatMaps/Link_matrix1000_major.csv") %>%
    normalizePath() %>%
    read.table(
        sep = ";",
        header = TRUE
...
GlobalFolding <- here("2_Derived/HeatMaps/Link_matrix100hydra_major.csv") %>%
    normalizePath() %>%
    read.table(
        sep = ";",
        header = TRUE
...
pltHeatmap_gFolding_sw100 <-
    here("2_Derived/HeatMaps/Link_matrix100hydra_major.csv") %>%
    read.table(
        sep = ";",
        header = TRUE
    ) %>%
...
stblt_df <-
    read.csv(here("2_Derived/HeatMaps/duplex-stablility_100x100.mtrx"),
        sep = ";",
        row.names = 1
    ) %>% .[, -dim(.)[2]]
...
cowplot::save_plot(
    ...
    base_asp = 0.9,
    file = normalizePath(
        file.path(plots_dir, "heatmap_folding_stability.svg")
    )
)
...
InvRepDens <-
    here("2_Derived/HeatMaps/Link_matrix_1000_invert_major_activ_left.csv") %>%
    normalizePath() %>%
    read.table(
        sep = ";",
        header = TRUE
...
# InvRepDens = read.table(here("2_Derived/HeatMaps/Link_matrix_direct_major_activ_left_cross.KpModifByHand.mtrx"), sep = '\t',header = TRUE, row.names = 1) # , row.names = NULL)
...
) # S = 1.0104e+10, p-value = 0.0003666 rho = 0.05628272
...
write.table(
    HomologyAndRepeats,
    here("3_Results/SlipAndJump.HomologyAndRepeats.txt") %>%
        normalizePath(),
    sep = "\t"
)
